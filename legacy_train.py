#! OLD CODE !#
# TODO rewrite with the nicer training api
import torch
...
if __name__ == "__main__":
    ...
    preprocessing_fn = BertPreprocess(
        bert=bert, tokenizer=bert_tokenizer, device=device
    )
    dataset = IronMarch(
    ...
    encoder = InterpolatedLinearLayers(
        input_dim, latent_dim_size * 2, num_layers=10, bias=1.0
    )
    decoder = InterpolatedLinearLayers(
        latent_dim_size, input_dim, num_layers=13, bias=1.0
    )
    feature_head = torch.nn.Linear(latent_dim_size, 7)
    model = VAE(
        encoder=encoder, decoder=decoder, feature_head=feature_head, use_softmax=softmax
    )
    model.to(device)
    ...
    dirichlet_distribution = torch.distributions.dirichlet.Dirichlet(
        torch.tensor([concentration_alpha for i in range(latent_dim_size)])
    )
    ...
    for epoch in range(epochs):
        ...
        for batch_num, batch in enumerate(progress_bar):
            ...
            augmented_posts = posts + torch.normal(
                mean=0.0, std=gaussian_std, size=posts.shape
            ).to(device)
            ...
            loss = (
                ...
                + distribution_loss * sinkhorn_weight
            ...
            tp_accum += (
                ((binary_predictions == 1.0) & (features == 1.0)).detach().sum().item()
            )
            fp_accum += (
                ((binary_predictions == 1.0) & (features == 0.0)).detach().sum().item()
            )
            tn_accum += (
                ((binary_predictions == 0.0) & (features == 0.0)).detach().sum().item()
            )
            fn_accum += (
                ((binary_predictions == 0.0) & (features == 1.0)).detach().sum().item()
            )

            accuracy = (tp_accum + tn_accum) / (
                tp_accum + fp_accum + tn_accum + fn_accum + epsilon
            )
        ...
        with torch.no_grad():
            for batch_num, batch in enumerate(progress_bar):
                ...
                sampled_dirichlet = dirichlet_distribution.sample([batch_size]).to(
                    device
                )
                distribution_loss = sinkhorn_loss_fn(eps, sampled_dirichlet)
                ...
                binary_predictions = torch.ge(feature_predictions, feature_threshold)
                tp_accum += (
                    ((binary_predictions == 1.0) & (features == 1.0))
                    .detach()
                    .sum()
                    .item()
                )
                fp_accum += (
                    ((binary_predictions == 1.0) & (features == 0.0))
                    .detach()
                    .sum()
                    .item()
                )
                tn_accum += (
                    ((binary_predictions == 0.0) & (features == 0.0))
                    .detach()
                    .sum()
                    .item()
                )
                fn_accum += (
                    ((binary_predictions == 0.0) & (features == 1.0))
                    .detach()
                    .sum()
                    .item()
                )
        ...
        torch.save(
            model,
            f"saved_models\\vae\\sinkhorn\\alpha_{concentration_alpha}_latent_{latent_dim_size}_softmax_{softmax}_version_{model_version}_features_{use_features}",
        )
