...
lr_scheduler = get_scheduler(
    name="linear", optimizer=optimizer, num_warmup_steps=100, num_training_steps=num_training_steps # TODO Change to add warmup
)
...
dirichlet_distribution = torch.distributions.dirichlet.Dirichlet(torch.tensor([config['dirichlet_alpha'] for i in range(config['latent_dim_size'])]))
...
for epoch in range(config['epochs']):
    ...
    for batch_num, batch in enumerate(p_bar):
        ...
        predicted_sequence_logits, embeddings = model_outputs.logits, model_outputs.embeddings
        del model_outputs

        # Calculate loss
        reconstruction_loss = reconstruction_loss_fn(predicted_sequence_logits.reshape(-1, model.vocab_size), data.view(-1))
        sampled_dirichlet = dirichlet_distribution.sample([config['batch_size']]).to(DEVICE) # TODO Figure out how many samples is appropriate
        distribution_loss = sinkhorn_loss_fn(embeddings, sampled_dirichlet)

        combined_loss = config['reconstruction_weight'] * reconstruction_loss + config['distribution_weight'] * distribution_loss
...
test_embeddings = torch.nn.functional.one_hot(torch.Tensor[5], num_classes=config['latent_dim_size'])
